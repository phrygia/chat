...
function Message({ message, user }) {
    const [getImage, setGetImage] = useState('https://raw.githubusercontent.com/phrygia/chat/master/client/src/assets/images/default_icon.png')
    const timeFromNow = (timestamp) => moment(timestamp).fromNow();
    const allFriendsList = useSelector((state) => state.user.currentUser.allFriendsList)
        
    useEffect(() => {
        allFriendsList.length > 0 && allFriendsList.filter((val) => {
    ...
    return (
        ...
        <Li className={user && message.user.id === user.uid ? "me" : "other"}>
            ...
            <div className="text_bubbles">
                {user && message.user.id !== user.uid && <strong>{message.user.name}</strong>}
                {isImage(message) ? (
